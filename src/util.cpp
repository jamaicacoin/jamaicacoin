...
const char * const BITCOIN_CONF_FILENAME = "jamaicacoin.conf";
const char * const BITCOIN_PID_FILENAME = "jamaicacoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "jamaicacoin" is a composite category enabling all JamaicaCoin-related debug output
                if(ptrCategory->count(std::string("jamaicacoin"))) {
...
#else
    const char* pszModule = "jamaicacoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JamaicaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JamaicaCoin
    // Mac: ~/Library/Application Support/JamaicaCoin
    // Unix: ~/.jamaicacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JamaicaCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JamaicaCoin";
#else
    // Unix
    return pathRet / ".jamaicacoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty jamaicacoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override jamaicacoin.conf
